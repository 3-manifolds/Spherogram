"""
Smoothing curves using Hobby's splines.

Mostly copied from code of Dylan Thurston.
"""
...
def hobby_dirs(points, tangent1=None, tangent2=None, curl1=1.0, curl2=1.0):
    ...
    if tangent1 is not None:
        ...
        matrices[0] = ((c + curl1 * a, d + curl1 * b, c, d),
                       (w + curl1 * v, w))
    ...
    if tangent2 is not None:
        ...
        matrices[-1] = ((a, b, a + curl2 * c, b + curl2 * d),
                        (v, v + curl2 * w))
    tridiag = accum_matrix(matrices)
    return solve_tri(*tridiag)
