...
import orthogonal
import hobby
# from sage.all import RR, vector, bezier_path, matrix, point2d, Graphics
...
def hobby_smooth(points):
    ...
    for k in range(len(points) - 1):
        (c0, c1) = hobby.good_bezier(points[k], dirs[k],
                                     dirs[k + 1], points[k + 1])
        if k == 0:
...
def test_gb(p1, a1, a2, p2, t1=1.0, t2=1.0):
    ...
    G = bezier_path([[p1, c0, c1, p2]]) + point2d(c0) + point2d(c1)
    # could point2d be used instead of point ?
